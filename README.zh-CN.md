...
- freeze 只会暂停命令执行，不会丢弃命令。冻结期间队列为空时，runner 仍可向 bot 请求决策，并把返回的命令保留到冻结结束后执行。
- freeze 在施放帧和下一帧生效。由于玩家 A 先于玩家 B 结算，玩家 B 后手施放 freeze 时，不会追溯取消玩家 A 在施放帧已经完成的动作。
...
- `freeze`：在 2 个生效帧内阻止敌方执行动作，但不丢弃已排队命令；冷却为 29 帧。
